...
LOG_DIR="/mnt/share/r00648901/sglang/logs/run_4node_pd_$(date +%Y%m%d_%H%M%S)"
mkdir -p "${LOG_DIR}"
echo "[run_4node_pd] logs will be saved to ${LOG_DIR}"
...
for i in "${!D_IPS[@]}"; do
  if [[ "$LOCAL_HOST1" == "${D_IPS[$i]}" || "$LOCAL_HOST2" == "${D_IPS[$i]}" ]]; then
    ...
    export SGLANG_SIMULATE_ACC_LEN=3.6
