...
class Indexer(MultiPlatformOp):
    ...
    def _get_q_k_bf16(
        ...
        # allgather+rerrange
        if forward_batch.cp_metadata is not None and self.nsa_enable_prefill_cp:
        ...
        if _is_cuda or _is_hip:
            ...
            if (
                ...
                if (
                    ...
                    kv_len_prev = forward_batch.cp_metadata.kv_len_prev
                    kv_len_next = forward_batch.cp_metadata.kv_len_next
                    actual_seq_q_prev = forward_batch.cp_metadata.actual_seq_q_prev
                    actual_seq_q_next = forward_batch.cp_metadata.actual_seq_q_next

                    # TODO support mutil-batch
                    # cp_batch_seq_index_prev = forward_batch.cp_metadata["cp_batch_seq_index_prev"]
                    # cp_batch_seq_index_next = forward_batch.cp_metadata["cp_batch_seq_index_next"]
    ...
    def forward_npu(
        ...
        if is_prefill:
            if self.nsa_enable_prefill_cp and forward_batch.cp_metadata is not None:
                forward_batch.attn_backend.forward_metadata.actual_seq_lengths_q = (
                    forward_batch.cp_metadata.actual_seq_q_prev_tensor,
                    forward_batch.cp_metadata.actual_seq_q_next_tensor,
                )
                forward_batch.attn_backend.forward_metadata.actual_seq_lengths_kv = (
                    forward_batch.cp_metadata.kv_len_prev_tensor,
                    forward_batch.cp_metadata.kv_len_next_tensor,
                )
