...
# DeepSeek-V4 DSpark BF16-draft correctness baseline on Ascend A3.
# Keep the target model and runtime arguments aligned with dsv4_dspark.sh;
# only the draft checkpoint and draft quantization mode differ.
...
# Keep the load audit enabled for the first BF16 baseline run.  Strict mode is
# left off because the BF16 draft intentionally has no quantization side tensors.
export SGLANG_DSPARK_QUANT_AUDIT=1
export SGLANG_DSPARK_QUANT_AUDIT_STRICT=0

TARGET_MODEL_PATH=/data/weights/DeepSeek-V4-Flash-DSpark-w4a8
DRAFT_MODEL_PATH=/data/weights/DeepSeek-V4-Flash-DSpark-bf16-draft

LOCAL_IP=$(hostname -I | awk '{print $1}')
echo "Launch W4A8 target + BF16 DSpark draft on ${LOCAL_IP}"
